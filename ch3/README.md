...
1. 什么时候应该使用大括号而非小括号
2. 为何别名声明优于typedef
3. constexpr和const有何不同
4. const成员函数和线程安全有何关系
5. ......

## 条款7. 在创建对象时注意区分()和{}

- 大括号初始化（也称统一初始化）可以应用的语境最为宽泛，且可以阻止隐式窄化类型转换（宽化不阻止）。还对解析语法免疫（指的是函数声明还是创建对象可能引起的歧义）
...
> 作者给出的建议是：选择()或{}并坚持下去，大部分可变模板内部采用小括号
...
- 避免在整型和指针类型之间重载（指的是函数形参为整型或指针，如：`void func(int)`和`void func(void *)` --- 其实都是为了照顾0或NULL的情况）
...
> 在C++11中很多模板别名还是利用`typedef`获得的，因此经常需要加上`typename`。（C++14都有对应的别名模板了，在原来的名字后面加上`_t`），但可以对C++11中的情况采用下述方式：
>
> ```cpp
> template<class T>
...
- 限定作用域的枚举类型和不限定范围的枚举类型都支持底层类型指定（例如：`enum Class Status: uint32_t`或者`enum Status: uint32_t`）。限定作用域的枚举类型的**默认底层类型是int**，而不限范围的枚举类型没有默认底层类型
...
> 1. 反正请都使用=delete吧，忘了private的方式～
>
> 2. 习惯上，删除函数会被声明为public，而非private
>
> 3. 删除模板中具体某种类型的实现
>
>    ```cpp
...
- 为需要改写的函数添加override声明（继承类的改写函数上面）
...
- 成员函数引用修饰词使得对于左值和右值对象(*this)的处理能够区分开来
  ...
  class Widget{
  public:
      void dowork() &;   // 左值Widget对象
      void dowork() &&;  // 右值Widget对象
  }
  ```

> 正是虚函数的改写，使得通过基类接口调用派生类函数成为了可能
>
> 改写(override)需要满足的条件：
>
> - 基类中的函数必须是虚函数（virtual修饰符）
...
> 1. 注意：const_iterator指的是"指针"所指的对象具有常量性，而不是指针本身具有常量性
> 2. 非成员函数(`cbegin(Iter类型),cend(Iter类型)`)在C++14中有，但C++11中还木有（但C++11提供的非成员函数版本的begin函数传入一个const容器会产生一个const_iterator）

## 条款14. 只要函数不会发射异常，就为其加上noexcept声明

- noexcept声明是函数接口的组成部分，这意味着调用方可能会对它有依赖（即调用方通过该声明来判断函数是否不会发射异常）
- 相对于不带noexcept声明的函数，带有noexcept声明的函数有更多机会得到优化（是指编译器层面的）
- noexcept性质对于移动操作，swap，函数释放函数和析构函数最有价值
- 大多数函数都是异常中立的，不具备noexcept性质

> 1. **优化诚可贵，正确价更高**
> 2. 内存释放函数和所有的析构函数（无论是用户定义的，还是编译期自动生成的）都隐式地具备noexcept性质
> 3. 这部分内容个人不是太清楚，可以进一步参考：[noexcept知乎](https://www.zhihu.com/question/30950837)，[noexcept stackflow](https://stackoverflow.com/questions/10787766/when-should-i-really-use-noexcept)（还是更偏向于上述给出的建议：① 默认不使用 ② 移动构造和移动赋值操作如果不抛出异常, 用noexcept ③ 简单的leaf function）
> 4. 《C++ Primer》上面给的建议是：一是我们确认函数不会抛出异常，二是我们根本不知道该如何处理异常

## 条款15. 只要有可能使用constexpr，就使用它

- constexpr**对象**都具备const属性，并由编译期已知的值完成初始化

- constexpr**函数**在调用时若传入的实参值是编译期已知的，则会产出编译期结果
...
> 其他关于constexpr的优势：
>
> 1. constexpr用于对象时，其实就是加强版的const（constexpr对象都是const对象，而并非所有的const对象都是constexpr对象）--- constexpr让编译器提供保证，让变量拥有一个值，用于要求编译期常量的语境
> 2. 编译期确定往往效率更高，应用语境更广（类似作为数组长度等等）
>
> **用户自定义类型**同样可能是字面值类型
>
> 在C++11中，constexpr函数都隐式地被声明为const（指成员函数的修饰词） --- 但C++14中没有此限制了
>
> ```cpp
> class Point {
> public:
...
- 运用`std::atomic`类型的变量会比运用互斥量提供更好的性能，但前者仅适用对单个变量或内存区域的操作（如果有两个或更多个变量或内存区域需要作为一整个单位进行操作时，需要采用互斥量）
...
- 特种成员函数是指那些C++会自动生成的成员函数：默认构造函数，析构函数，复制操作，以及移动操作（C++11）--- 仅当一个类没有声明任何构造函数时，才会生成默认构造函数
...
>
> **复制构造函数**：按成员进行非静态数据成员的复制构造。仅当类中不包含用户声明的复制构造函数时才生成。如果该类声明了移动操作，则复制构造函数将被删除。在已经存在复制赋值运算符或析构函数的条件下，仍然生成复制构造函数已经成了被废弃的行为
>
> **复制赋值运算符**：按成员进行非静态数据成员的复制赋值。仅当类中不包含用户声明的复制赋值运算符时才生成。如果该类声明了移动操作，则复制构造函数将被删除。在已经存在复制构造函数或析构函数的条件下，仍然生成复制赋值运算符已经成了被废弃的行为
>
> **移动构造函数和移动赋值运算符**：都按成员进行非静态数据成员的移动操作。仅当类中**不包含用户声明的复制操作，移动操作和析构函数时才生成**。
>
> ② 大三定律：如果你声明了复制构造函数，复制赋值运算符或析构函数中的任何一个，你就得同时声明所有这三个（其实也可以加一个移动操作）
>
> ③ 生成的特种成员函数都具有public访问层级且是inline的，而且它们都是非虚的
>
> ④ 两种移动操作(移动构造函数和移动赋值操作)并不彼此独立：声明了其中一个，就会阻止编译器生成另一个
